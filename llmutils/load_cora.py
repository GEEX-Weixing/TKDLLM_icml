...
def get_cora_casestudy(SEED=0):
    ...
    val_idx = remaining_idx[: 30]
    test_idx = remaining_idx[30: 90]

    # 确保我们有足够的节点来分配给验证集和测试集
    assert len(val_idx) == 30, "Not enough nodes for validation set"
    assert len(test_idx) == 60, "Not enough nodes for test set"
...
def parse_cora():
    path = 'dataset/dataset/cora/cora'
...
def get_raw_text_cora(use_text=False, seed=0):
    ...
    with open('dataset/dataset/cora/mccallum/cora/papers') as f:
        lines = f.readlines()
    pid_filename = {}
    for line in lines:
        pid = line.split('\t')[0]
        ...
        pid_filename[pid] = fn
    path = 'dataset/dataset/cora/mccallum/cora/extractions/'
